...
cross.validation <- function(ref.exp, modules, eig.exp, power, gene.exp.min=0.5, gene.exp.max=10, num.genes=3, seed=123, ..., replace.unidentifiable=TRUE){
  ...
  for (n in seq_len(nrow(ref.exp))){
    ...
    for (g in seq_len(length(low.genes))){
      ...
      set.seed(46)
      predicted.values <- list.append(predicted.values, predict(training, node=gene, data=sub.test, method='bayes-lw'))
      predicted.genes <- c(predicted.genes, gene)
    }

    predicted.matrix <- matrix(unlist(predicted.values), nrow=length(predicted.values), byrow=TRUE)
    rownames(predicted.matrix) <- predicted.genes
    colnames(predicted.matrix) <- rownames(test.data)

    true.data <- as.matrix(t(test.data[,colnames(test.data) %in% rownames(predicted.matrix)]))
    nrmse <- NRMSE(true.data, imputed.data=predicted.matrix)
